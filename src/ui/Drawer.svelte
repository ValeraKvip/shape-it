...
<div
	...
	class="fixed right-0 top-0 z-40 h-screen w-full max-w-2xl -translate-x-full transform-none overflow-y-auto p-4 pb-0 transition-transform dark:bg-gray-800 flex flex-col"
	tabindex="-1"
...
>
	...
	<div
		class="bottom-0 left-0  flex w-full justify-center space-x-4 p-4  sm:mt-4 sm:px-4 sticky dark:bg-gray-800 z-50"
	>
		{#if isCreateMode}
			<button
	...
	<div class:hidden={!showDeletionModal} class="overflow-y-auto overflow-x-hidden fixed flex top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
		<div class="relative p-4 w-full max-w-md max-h-full ">
			<div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
				<button onclick={()=>showDeletionModal = false} type="button" class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="popup-modal">
				...
				<div class="p-4 md:p-5 text-center">
					...
					<button onclick={()=>{showDeletionModal = false; submitDelete()}} data-modal-hide="popup-modal" type="button" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center">
						Delete
					</button>
					<button onclick={()=>showDeletionModal = false}  type="button" class="py-2.5 px-5 ms-3 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">Cancel</button>
				</div>
			</div>
		</div>
	</div>
